...
if [ "$#" -eq 0 ]; then
  set -- frontend-service backend-service agent-service application-service
fi
for batch_service in "$@"; do
  case "$batch_service" in
    frontend-service|backend-service|agent-service|application-service) ;;
    *) printf 'Unknown service: %s\n' "$batch_service" >&2; exit 1 ;;
  esac
done
...
printf 'Pushed %s image(s) with release tag %s\n' "$#" "$IMAGE_TAG"
