...
resource "aws_ssm_parameter" "cicd_init" {
  ...
  value = jsonencode({ db_resource_id = aws_db_instance.oracle.resource_id, status = "unconfirmed" })
  lifecycle {
...
resource "aws_iam_role" "cicd" {
  ...
  lifecycle {
    precondition {
      condition     = can(regex("^[0-9]+$", var.github_owner_id)) && can(regex("^[0-9]+$", var.github_repository_id)) && var.cicd_state_bucket != ""
...
resource "aws_iam_role_policy" "cicd" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    {
      Sid = "TerraformResourceDiscovery", Effect = "Allow", Resource = "*"
      Action = [
        "ec2:Describe*", "elasticloadbalancing:Describe*", "rds:Describe*", "rds:ListTagsForResource",
        "ecs:Describe*", "ecs:List*", "ecr:Describe*", "ecr:List*", "ecr:GetRepositoryPolicy", "ecr:GetLifecyclePolicy",
        ...
        "servicediscovery:GetNamespace", "servicediscovery:GetService", "servicediscovery:List*",
        "route53:GetHostedZone", "route53:ListResourceRecordSets", "route53:ListTagsForResource", "route53:GetChange",
        "acm:DescribeCertificate", "acm:ListTagsForCertificate"
      ]
    },
    {
      Sid      = "ReadTerraformIAMResources", Effect = "Allow"
      Action   = ["iam:GetRole", "iam:GetRolePolicy", "iam:ListRolePolicies", "iam:ListAttachedRolePolicies", "iam:ListRoleTags", "iam:GetOpenIDConnectProvider"]
      Resource = concat(local.cicd_roles, [aws_iam_role.cicd[0].arn, local.cicd_provider], [for r in aws_iam_role.cicd_infrastructure : r.arn])
    },
    {
      Sid      = "ReadProjectBuckets", Effect = "Allow", Action = ["s3:Get*", "s3:ListBucket"]
      Resource = concat(["arn:aws:s3:::${var.cicd_state_bucket}", aws_s3_bucket.alb_logs.arn], [for b in aws_s3_bucket.cicd_plans : b.arn])
    },
    {
      Sid      = "TerraformState", Effect = "Allow", Action = ["s3:GetObject", "s3:PutObject"]
      Resource = ["arn:aws:s3:::${var.cicd_state_bucket}/${var.cicd_state_key}"]
    },
    {
      Sid      = "TerraformLock", Effect = "Allow", Action = ["s3:GetObject", "s3:PutObject", "s3:DeleteObject"]
      Resource = ["arn:aws:s3:::${var.cicd_state_bucket}/${var.cicd_state_key}.tflock"]
    },
    {
      Sid = "ECRLogin", Effect = "Allow", Action = ["ecr:GetAuthorizationToken"], Resource = "*"
    },
    {
      Sid      = "PushProjectImages", Effect = "Allow"
    ...
    {
      Sid = "RegisterTaskDefinitions", Effect = "Allow", Action = ["ecs:RegisterTaskDefinition"], Resource = "*"
    },
    {
      Sid      = "ManageProjectTaskRevisions", Effect = "Allow"
      Action   = ["ecs:DeregisterTaskDefinition", "ecs:TagResource", "ecs:UntagResource"]
      Resource = [for suffix in ["frontend", "backend", "app1", "app2", "init"] : "${local.cicd_prefix}:task-definition/${local.name}-${suffix}:*"]
    },
    {
      Sid      = "DeployProjectServices", Effect = "Allow"
      Action   = ["ecs:CreateService", "ecs:UpdateService", "ecs:TagResource", "ecs:UntagResource"]
      Resource = [for suffix in ["frontend", "backend", "app1", "app2"] : "${local.cicd_prefix}:service/${local.name}/${local.name}-${suffix}"]
    },
    {
      Sid       = "RunPrivateDatabaseInitializer", Effect = "Allow", Action = ["ecs:RunTask"]
      Resource  = ["${local.cicd_prefix}:task-definition/${local.name}-init:*"]
      Condition = { ArnEquals = { "ecs:cluster" = aws_ecs_cluster.main.arn } }
    },
    {
      Sid       = "PassOnlyProjectTaskRoles", Effect = "Allow", Action = ["iam:PassRole"]
      Resource  = local.cicd_roles
      Condition = { StringEquals = { "iam:PassedToService" = "ecs-tasks.amazonaws.com" } }
    },
    {
      Sid      = "RuntimeSecretInitialization", Effect = "Allow"
      Action   = ["secretsmanager:DescribeSecret", "secretsmanager:GetResourcePolicy", "secretsmanager:ListSecretVersionIds", "secretsmanager:GetSecretValue", "secretsmanager:PutSecretValue"]
      Resource = [aws_secretsmanager_secret.runtime.arn]
    },
    {
      Sid      = "DatabaseInitializationCheckpoint", Effect = "Allow"
      Action   = ["ssm:GetParameter", "ssm:PutParameter", "ssm:ListTagsForResource"]
      Resource = [aws_ssm_parameter.cicd_init[0].arn, aws_ssm_parameter.cicd_revision[0].arn]
    },
    {
      Sid      = "EnableExistingTargetAlarms", Effect = "Allow", Action = ["cloudwatch:PutMetricAlarm"]
      Resource = [for suffix in ["frontend", "backend"] : "arn:aws:cloudwatch:${var.region}:${local.cicd_account}:alarm:${local.name}-${suffix}-healthy-hosts"]
    }
  ] })
}
...
output "cicd_task_definitions" {
  value = merge(
    { for k, t in aws_ecs_task_definition.core : "${local.name}-${k}" => t.arn },
    { for k, t in aws_ecs_task_definition.app : "${local.name}-${k}" => t.arn }
  )
}
