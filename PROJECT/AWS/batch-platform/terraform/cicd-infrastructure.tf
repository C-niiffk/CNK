...
locals {
  ...
  cicd_read_statements = flatten([
    for p in aws_iam_role_policy.cicd : [
      ...
      if contains(
        [
          "TerraformResourceDiscovery",
          "ReadTerraformIAMResources",
          "ReadProjectBuckets",
          "TerraformLock"
        ],
        s.Sid
...
# Saved plans are private, encrypted, and automatically expire.
# They are not public GitHub artifacts.
  ...
  policy = jsonencode({
    ...
    Statement = concat(
      ...
      [
        ...
        # DescribeParameters does not support restricting access to
        # individual SSM parameter ARNs, so it requires Resource = "*".
        # This is read-only metadata access used by Terraform while
        # refreshing aws_ssm_parameter resources during plan.
        {
...
resource "aws_iam_role_policy" "cicd_apply" {
  ...
  policy = jsonencode({
    ...
    Statement = concat(
      [
        ...
        if contains(
          [
            ...
            "ManageProjectTaskRevisions",
            "DeployProjectServices",
            "PassOnlyProjectTaskRoles",
            "EnableExistingTargetAlarms"
          ],
          s.Sid
        )
      ],
      [
        {
          Sid    = "ReadApprovedPlan"
          Effect = "Allow"
          Action = [
            "s3:GetObject"
          ]
          Resource = [
            "${aws_s3_bucket.cicd_plans[0].arn}/plans/*"
          ]
        },

        {
          Sid    = "ModifyExistingOracle"
          Effect = "Allow"
          Action = [
          ...
          Resource = [
            aws_db_instance.oracle.arn
          ]
        },

        {
          Sid    = "ModifyDatabaseNetwork"
          Effect = "Allow"
          Action = [
          ...
          Resource = [
            ...
            aws_db_option_group.oracle.arn
          ]
        },

        {
          Sid      = "RegionalNetworkChanges"
          Effect   = "Allow"
          Resource = "*"

          # Network creation APIs need wildcard resources.
          # This is a lab role for the selected region.
          # A production account should further restrict these actions by
          # VPC, resource tags, and permission boundaries.
          Condition = {
            StringEquals = {
        ...
        {
          Sid    = "ModifyProjectLoadBalancing"
          Effect = "Allow"
          Action = [
          ...
          Resource = concat(
            [for b in aws_lb.main : b.arn],
            [for t in aws_lb_target_group.main : t.arn]
          )
        },

        {
          Sid    = "ModifyProjectLogs"
          Effect = "Allow"
          Action = [
          ...
          Resource = [
            "arn:aws:logs:${var.region}:${local.cicd_account}:log-group:/ecs/${local.name}/*"
          ]
        },

        {
          Sid    = "ModifyProjectAlarms"
          Effect = "Allow"
          Action = [
          ...
          Resource = [
            "arn:aws:cloudwatch:${var.region}:${local.cicd_account}:alarm:${local.name}-*"
          ]
        }
      ]
    )
...
output "cicd_plan_role_arn" {
  value = try(aws_iam_role.cicd_infrastructure["plan"].arn, "")
}

output "cicd_infra_role_arn" {
  value = try(aws_iam_role.cicd_infrastructure["apply"].arn, "")
}

output "cicd_plan_bucket" {
  value = try(aws_s3_bucket.cicd_plans[0].id, "")
}

output "cicd_revision_parameter" {
  value = try(aws_ssm_parameter.cicd_revision[0].name, "")
}

output "cicd_pr_plan_role_arn" {
  value = try(aws_iam_role.cicd_infrastructure["pr-plan"].arn, "")
}
